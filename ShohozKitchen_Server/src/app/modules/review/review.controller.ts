...
const ReviewController = {
    ...
    canReview: catchAsync(async (req: Request, res: Response) => {
        const result = await ReviewService.canReview(req.user!.userId, req.params.productId);
        sendResponse(res, { statusCode: 200, success: true, message: 'Eligibility checked', data: result });
    }),
    ...
    update: catchAsync(async (req: Request, res: Response) => {
        ...
        const review = isAdmin
            ? await ReviewService.adminUpdateReview(req.params.id, req.body)
            : await ReviewService.updateReview(req.params.id, req.user!.userId, req.body);
