            ...
            const fanOut = async () => {
                ...
                for (const admin of admins) {
                    await NotificationService.notify({
                        ...
                        link: '/dashboard/admin/returns/' + returnIdStr,
...
// ── Approve ────────────────────────────────────────────────────
    async approveReturn(id: string, resolver: { userId: string }, note?: string) {
        ...
        // Reflect on the order: mark the package as returned
        await OrderService.markPackageReturned(doc.order, note);
        ...
        try {
            ...
            NotificationService.notify({
                ...
                link: '/dashboard/user/returns/' + doc._id.toString(),
    ...
    async refundReturn(id: string, resolver: { userId: string }, note?: string) {
        ...
        // Reflect on the order: mark the package as refunded + flip payment status
        await OrderService.markPackageRefunded(doc.order, note);
